...
### fingerprint dedup 已改为动态（历史遗留文档，已修复）
`scheduler.py` 里 `_ingest_data` 的 fingerprint 已从硬编码 `(title, js_code[:100])` 改为动态构建：
```python
key_fields = (
...
```
通用于任何网站。但当 `target_fields` 只有 1 个字段时（如只有 title），不同记录碰巧 title 相同会被误判为重复。

### content_filter 是 Gemini 的问题（无自动重试）
日志里频繁出现 `finish_reason=content_filter` 是 Gemini 的安全过滤，不是 bug。
**注意**：controller.py **没有** content_filter 重试逻辑。LLM client 层（llm.py）对 API 级错误有重试，但 `content_filter` 作为正常响应的 `finish_reason` 不会触发重试。如果 Step 0 就触发 content_filter，该 session 会收到一个空 decision（无 tool_calls），controller 直接 break，session 0 步完成。这在 Gemini + 复杂 prompt 场景下是常见失败模式。
...
**关键行为**：
1. Explorer prompt 里 **建议** 写 `site_model`，但仅是 prompt 指令。B1.2 增加了 orchestrator 层的硬校验：burst loop 结束后检查 `site_model` 是否有 `structure` 或 `estimated_total > 0`，无效则运行 10 步补救 Explorer。
2. Explorer 的采样记录不再丢弃，保存为黄金标准。`_js_extract_save` 在保存前做结构验证（非 LLM）：字段是否匹配 schema？长字段是否异常短？关键字段值是否出现在 page_html 中（防幻觉）？
3. **覆盖率检查在 orchestrator 层，不在 CompletionGate 内部**：`gate.py` 的 `CompletionGate.check()` 只检查 count + quality；覆盖率检查（`estimated_total > 20 且 coverage < 10%`）写在 `orchestrator._run_full_site()` 的调用处。
4. **Inter-round feedback（B1.1）**：`_compute_round_report()` 在每轮提取后生成结构化反馈（frontier 状态、未采样 section、per-section 记录数、质量问题），注入下一轮 Explorer 的 `knowledge_summary`。
5. **Per-section coverage（B1.4）**：`SharedFrontier` 追踪每个 section 的提取记录数。URL 通过路径前缀启发式关联到 section。
6. **DataVerifier mid-pipeline（B1.3）**：每轮提取后自动运行 DataVerifier，结果纳入 round_report 反馈给 Explorer。

**指纹去重修复**（`scheduler.py:377`）：由硬编码 `(title, js_code[:100])` 改为用 `spec.target_fields[:3]` 动态构建，通用于任何网站。
...
## 已实现 vs 规划中（重要：防止在虚假前提上开发）

以下明确区分当前代码的实际状态和文档中的愿景设计。

### ✅ 已实现并在代码中工作
- 三阶段执行：Phase 0 → Explorer burst loop → Extractor 串行提取
- SharedFrontier URL 状态机（QUEUED → IN_FLIGHT → EXTRACTED/SAMPLED/FAILED）
- Navigate 层 URL 去重
- 侧信道（save_records, report_urls, report_sections）
- Extractor pre-navigation（orchestrator 在 session 前 navigate）
- proven_scripts + hard-replay（URL pattern → JS 直接执行）
- proven_scripts merge-not-replace
- RunIntelligence 文件持久化（run_knowledge.json + golden_records.json）+ 原子写入
- StructuralCompletionGate 已激活（B2.1）：burst loop 中覆盖 Explorer 的 TASK COMPLETE（sections 未采样时强制继续）
- Section-aware 提取优先级（B2.2）：`next()` 优先选来自零记录 section 的 URL
- ContextManager token budget 执行（B2.3）：`build()` 超限时 `_trim_to_budget()` 裁剪最老 history
- 动态 fingerprint 去重（spec.target_fields[:3]）
- Governor 软 nudge + 硬 stop
- URL 规范化统一函数（`src/utils/url.py:normalize_url`）
- URL pattern 匹配全路径锚定（`^/seg/*/seg$`，非子串）
- `_extract_one()` 异常保护（try/catch 在调用点，mark_failed + 继续）
- 崩溃时保存部分结果（frontier.all_data() → partial_data）
- `_run_sampling_loop` 已激活（burst loop 后、final drain 前调用）
- content_filter 重试（controller 最多重试 3 次，非正常完成）
- DataVerifier 自动运行：最终结果验证 + 每轮提取后 mid-pipeline 验证（B1.3）
- SharedFrontier 公共 API（`get_status()` 替代 `_records` 私有访问）
- search_tool 域名过滤（精确匹配 + 子域名，非子串）
- Per-section coverage tracking（B1.4）：`register_section` / `associate_url_with_section` / `section_coverage`
- Inter-round feedback（B1.1）：`_compute_round_report` → `knowledge_summary` 注入 Explorer
- site_model 验证与补救（B1.2）：burst loop 后检查 site_model，无效则运行 10 步补救 Explorer
- Hard-replay quality metrics（B3.1）：`record_success` tracks `attempts`; `record_hard_replay_failure` tracks `failures` + `recent_failures`; `get_script_for_url` skips degraded patterns（attempts≥3, failures≥3, rate<0.5）
- Hard-replay golden schema validation（B3.2）：`validate_records` after `_has_extractable_content`, fails → fallback to LLM
- Hard-replay failure tracking（B3.3）：exception/null/no-content paths all call `record_hard_replay_failure` → feeds skip logic
- Enriched prior experience（B3.4）：`_update_experience` helper, format includes method (HARD-REPLAY/LLM) + fields filled count
- Phase 1.5 Listing Sampler：`_run_listing_sampler()` 在 burst loop 后、final drain 前运行，非 LLM 处理 entry_points 的 listing 页，提取 detail 链接注入 frontier

### ⚠️ 部分实现 / 行为与文档不符
- **覆盖率检查**：写在 orchestrator 调用处，不在 CompletionGate 内部

### ❌ 规划中 / 未实现（文档描述但代码中不存在）
- **并发 Extractor**（`concurrent-exploration-architecture.md` Stage 3-4）：代码仍是串行 `await _extract_one()`
- **DB 是 source of truth**（`exploration-memory-architecture.md`）：SharedFrontier 仍是内存 dict，DB 是可选的写入目标
- **Explorer = 站点架构师**（`concurrent-exploration-architecture.md` §1.1）：Explorer 是普通 CrawlController，无强制 think-after-extract
- **Episodic 记忆 / WebCoach**（`exploration-memory-architecture.md` §4）：sessions 表存在但从未被后续 session 读取
- **语义层 / pgvector**（`exploration-memory-architecture.md` §3.3）：完全未实现
- ~~**Phase 1.5 Listing Sampler**（`structure-mapping-redesign.md`）~~：✅ 已实现
- **Phase 0 VerifiedListings**（`structure-mapping-redesign.md`）：未实现
- **report_listing_urls() 侧信道**（`structure-mapping-redesign.md`）：未实现
- **known_scripts DB 表**（`concurrent-exploration-architecture.md` §4.3）：未实现，仍用 JSON 文件

### 已知 Bug / 架构缺陷（待修复）
- 浏览器 SPA DOM 稳定检测 — evaluate 本身无超时保护（循环有 15 次上限）
